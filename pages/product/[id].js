...
//   const res = await fetch(`https://backend-five-teal.vercel.app/api/v1/product/${id}`);
...
const ProductDetails = () => {
  ...
  const { data, refetch, isSuccess, isLoading } = useQuery({
    ...
    queryFn: () =>
      fetch(
        `https://backend-five-teal.vercel.app/api/v1/product/${id}`
      ).then((res) => res.json()),
  });
                ...
                {product === null ? <><CustomProductDetailsSkeleton /></> : <>
                  ...
                  <div className="md:col-span-7 col-span-12 pt-0  md:pt-14 p-2  md:mr-10 mr-0">
                    ...
                    <div className="flex  items-end gap-2">
                      ...
                      />
                      {product?.quantity > 0 ? (
                        <p className="text-green-600 font-bold mt-1"> In Stock</p>
                      ) : (
                    ...
                    <div className="mt-10 grid grid-cols-2  gap-4 ">
                      ...
                      >
                        ...
                        } onClick={handleBuyNowButtonClick} className="btn btn-primary w-full  rounded flex items-center gap-2  text-base-100 "><BsBagPlus size={22} /><h1>Buy Now</h1></button>
                      </Link>
                      <button
                        onClick={() => { handleSetLocalStorage(); handleAddCartButtonClick() }}
                        className="btn bg-[#ff8b2c] hover:bg-[#D0611E] border-none w-full flex items-center gap-2 rounded  text-base-100 "
